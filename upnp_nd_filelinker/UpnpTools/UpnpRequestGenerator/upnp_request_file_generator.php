...
foreach($desc_xml->xpath('//upnp:device') as $device){
   ...
   mkdir(str_replace(":", "_","$host_name/$device_name"));
   ...
   foreach($device->serviceList->service as $service){
      ...
      mkdir(str_replace(":", "_","$host_name/$device_name/$service_id"));
      ...
      foreach($service_desc->xpath('//upnp:action') as $action){
         ...
         $action_body .=
            ...
            "   </s:Body>\n".
            "</s:Envelope>";
         $fh = fopen(str_replace(":", "_","$host_name/$device_name/$service_id/$action_name"),"w");
         fwrite($fh,
